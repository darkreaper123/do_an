...
from model.GIFT4Rec_advanced.GIFT4Rec_advanced_kgat import GIFT4Rec_advanced_kgat
...
def get_dict_id_params(model, task_id):
    ...
    for index, (name, params) in enumerate(model.named_parameters()):
        if name == 'mask_bias_'.format(true_task_id) or name == 'mask_weight_'.format(true_task_id):
            source_dict[index - 2] = params.data
    return source_dict    
...
def train(args):
    ...
    item_ids = torch.arange(n_items, dtype=torch.long).to(device)
    model =       GIFT4Rec_advanced_kgat(args, data.n_users, data.n_entities, data.n_relations,
    ...
    for epoch in range(1, args.n_epoch + 1):
        ...
        if not args.training_embed_weight:
          args.use_weight_embeds_no_grad = 1
        time8 = time()
