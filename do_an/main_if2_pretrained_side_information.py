...
from model.IF2_pretrained_side_information import IF2_pretrained_side_information
...
def get_dict_id_params(model, task_id):
    ...
    for index, (name, params) in enumerate(model.named_parameters()):
        #print(index)
        #print(name)
        if 'mask_bias_{}'.format(true_task_id) in name or 'mask_weight_{}'.format(true_task_id) in name:
            source_dict[index + (-1 if true_task_id == 1 else 1)] = params.data
    return source_dict    
...
def train(args):
    ...
    item_ids = torch.arange(n_items, dtype=torch.long).to(device)
    model = IF2_pretrained_side_information(args, data.n_users, data.n_entities, data.n_relations,
    ...
    for epoch in range(1, args.n_epoch + 1):
        model.using_emb_one_time = False
        ...
        if True:
            if True:
              ...
              if metrics_list[k_max]['recall'].index(best_ndcg) == len(epoch_list) - 1:
                ...
                users_emb = all_emb[-n_users:].detach().cpu().numpy()
                items_emb = all_emb[:n_items].detach().cpu().numpy()
                np.savetxt(f"ml_1m_for_dropout_net/trained/warm/U_embedding_{args.index_embedding_file}.txt", users_emb)
                np.savetxt(f"ml_1m_for_dropout_net/trained/warm/V_embedding_{args.index_embedding_file}.txt", items_emb)
              if should_stop:
                #break
                pass
        if not args.ensemble_embed_features:
